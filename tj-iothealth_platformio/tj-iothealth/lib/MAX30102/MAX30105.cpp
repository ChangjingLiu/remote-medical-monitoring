...
//
#include <Arduino.h>
#define LED 2 //GPIO 2 为esp32-12F的内置LED
//
// Status Registers
...
//在Initial的结尾加入shutdown()主动进入休眠模式
void MAX30105::Initial(void)
{
  //血氧传感器的初始化
  //MAX30105 sensor;
  //Serial.println("SPO2 meter setting");
  ...
  //参数设置
  byte ledBrightness = 28; // 0 = off,  255 = 50mA   //示例30
  byte sampleAverage = 4;  // 1, 2, 4, 8, 16, 32      //示例4
  byte ledMode = 2;        //这个选项不用变  1 = Red only, 2 = Red + IR(MAX30201), 3 = Red + IR + Green (MAX30105 only)
  int sampleRate = 100;    //采样率： 50, 100, 200, 400, 800, 1000, 1600, 3200 //示例200
  int pulseWidth = 411;    // 脉宽69, 118, 215, 411                        //示例411
  int adcRange = 2048;     // 输入范围 2048, 4096, 8192, 16384                   //示例2048

  MAX30105::setup(ledBrightness, sampleAverage, ledMode, sampleRate, pulseWidth, adcRange);
  //MAX30105::setup();
  MAX30105::getINT1(); // 通过读取清除状态寄存器
  MAX30105::getINT2();
  //numSamples = 0;
  /////////休眠
  MAX30105::shutDown();
...
//在getSpO2的开始加入wakeup()结束休眠，在测量结束shutdown()进入休眠
void MAX30105::getSpO2(float *n_spo2, int32_t *n_heart_rate)
{
  MAX30105::wakeUp();
  //delay(500);
  for (int i = 0; i < 100; i++)
...
//RFA_BUFFER_SIZE 可以更改
// 采样率为100，将实际采样率的4个样本合1，因此采样率为25 ，由于RFA_BUFFER_SIZE=100，因此收集一次需要4秒钟
#define RFA_BUFFER_SIZE 100
  ...
  ///////
  for (byte i = 0; i < RFA_BUFFER_SIZE; i++)
  {
    ...
    //交换IR/RED寄存器内容
    aun_red_buffer[i] = MAX30105::getFIFOIR(); //硬件设计问题，换
    aun_ir_buffer[i] = MAX30105::getFIFORed(); //硬件设计问题，换
  ...
  rf_heart_rate_and_oxygen_saturation(aun_ir_buffer, RFA_BUFFER_SIZE, aun_red_buffer, n_spo2, &ch_spo2_valid, n_heart_rate, &ch_hr_valid, &ratio, &correl, &xy_ratio);
...
PA MAX30105::getSpO2_test()
{
  ...
  a->n_heart_rate = 99;
  a->n_spo2 = 99;
  rf_heart_rate_and_oxygen_saturation(aun_ir_buffer, RFA_BUFFER_SIZE, aun_red_buffer, &a->n_spo2, &ch_spo2_valid, &a->n_heart_rate, &ch_hr_valid, &ratio, &correl, &xy_ratio);
  // toggle the board LED. This should happen every ST (= 4) seconds if MAX30102 has been configured correctly
  //digitalWrite(LED, !digitalRead(LED));
  /*
  //////测试
  ...
  int beatAvg = 0;
  for (int i = 0; i < RFA_BUFFER_SIZE; i++)
  {
    long irValue = aun_red_buffer[i];
